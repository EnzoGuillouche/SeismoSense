 ...
 */
bool CameraPerspectiveDemo::setupWindow(const char* window_name) {
    ...
    window = glfwCreateWindow(gl_viewport_w, gl_viewport_h, window_name, NULL, NULL);
    if(!window) {
        glfwTerminate();
...
int CameraPerspectiveDemo::run(const char* window_name) {
     ...
     */
    string vertex_shader_str = shader_loader.load("Shaders/camera_perspective_demo.vert");
    string fragment_shader_str = shader_loader.load("Shaders/camera_perspective_demo.frag");
